...
namespace PenumbraMod.Content.Items
{
    ...
    public class PrismaRepeaterProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (counter > 110)
            {
                Vector2 velocity = (Main.MouseWorld - Projectile.Center) / 10f;
                int basic = Projectile.NewProjectile(Projectile.InheritSource(Projectile), owner.Center, velocity, ModContent.ProjectileType<PrismShotGlow>(), 1, 0, owner.whoAmI);
                int basic2 = Projectile.NewProjectile(Projectile.InheritSource(Projectile), owner.Center, velocity, ModContent.ProjectileType<PrismShotWhite>(), 1, 0, owner.whoAmI);
                int basic3 = Projectile.NewProjectile(Projectile.InheritSource(Projectile), owner.Center, velocity, ModContent.ProjectileType<PrismShot>(), 70, 0, owner.whoAmI);
                counter = 100;
    ...
    public class PrismShotGlow : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Prisma Bullet"); // By default, capitalization in classnames will add spaces to the display name. You can customize the display name here by uncommenting this line.
        ...
        {
            Color value = Main.hslToRgb((progressOnStrip * 1.6f - Main.GlobalTimeWrappedHourly) % 1f, 1f, 0.5f);
            Color result = Color.Lerp(Main.DiscoColor, value, Utils.GetLerpValue(-0.2f, 0.5f, progressOnStrip, clamped: true)) * (1f - Utils.GetLerpValue(0f, 0.98f, progressOnStrip));
            result.A = 0;
            return result;
        }
    ...
    public class PrismShotWhite : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Prisma Bullet"); // By default, capitalization in classnames will add spaces to the display name. You can customize the display name here by uncommenting this line.
