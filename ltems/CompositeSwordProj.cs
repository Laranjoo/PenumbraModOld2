...
{
	public class CompositeSwordProj : ModProjectile
	{
		...
		public override void AI() {
			...
			if (Timer == swingTime / 2) {
				...
				SoundEngine.PlaySound(new SoundStyle("PenumbraMod/Assets/Sounds/Items/CompositeSwordHit") {
                    Volume = 1.6f,
                    PitchVariance = 0.3f,
                    MaxInstances = 2,
		...
		private void DrawLine(List<Vector2> list) {
			...
			for (int i = 0; i < list.Count - 1; i++) {
				...
				Color color = Lighting.GetColor(element.ToTileCoordinates(), PenumbraMod.CompositeSword);
