        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int i = 0; i < Projectile.oldPos.Length; i++)
            {
                ...
                for (float j = 0; j < 1; j += 0.0625f)
                {
                    ...
                    Main.EntitySpriteDraw(texture, lerpedPos, null, Color.White * 2.3f * (1 - ((float)i / (float)Projectile.oldPos.Length)), lerpedAngle, new Vector2(texture.Width / 2, texture.Height / 2), 1, SpriteEffects.None, 0);
                }
            }
            return true;
        }
