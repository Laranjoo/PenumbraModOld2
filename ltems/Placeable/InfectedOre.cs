...
namespace PenumbraMod.Content.Tiles
{
	public class InfectedOre : ModTile
	{
		public override void SetStaticDefaults() {
			...
			DustType = 84;
			ItemDrop = ModContent.ItemType<Items.Placeable.InfectedOre>();
        ...
        public override void OnKill(NPC npc)
        {
            if (npc.type == NPCID.TheDestroyer && NPC.downedMechBoss1)
            {
                ...
                if (ShiniesIndex != -1)
                {
                    ...
                    tasks.Insert(ShiniesIndex + 1, new InfectedPass("Infected Ore", 237.4298f));
                }
            } 
        }

    }
    public class InfectedPass : GenPass
    {
        public InfectedPass(string name, float loadWeight) : base(name, loadWeight)
        {
        }
