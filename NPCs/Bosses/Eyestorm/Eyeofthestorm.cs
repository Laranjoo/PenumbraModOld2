...
namespace PenumbraMod.Content.NPCs.Bosses.Eyestorm
{
    ...
    public class Eyeofthestorm : ModNPC
    {
        ...
        private void FirstPhase(Player player)
        {
            ...
            if (NPC.localAI[0] >= 480 && NPC.localAI[0] <= 539)
            {
                NPC.friendly = false;
                float range2 = 2500f * 16f; // 100 tiles
                if (NPC.DistanceSQ(player.Center) < range2 * range2)
            ...
            if (NPC.localAI[1] >= 510 && NPC.localAI[1] <= 540)
            {
                NPC.alpha += 20;
            }
            // EFFECT
            ...
            // PROJECTILES
            if (NPC.localAI[1] == 560 || NPC.localAI[1] == 580 || NPC.localAI[1] == 600 || NPC.localAI[1] == 620 || NPC.localAI[1] == 640)
            {
            ...
            // BEAM PROJECTILES
            if (NPC.localAI[1] == 760 || NPC.localAI[1] == 820 || NPC.localAI[1] == 880)
            {
                NPC.dontTakeDamage = false;
                var entitySource = NPC.GetSource_FromAI();
                NPC.friendly = false;
            ...
            if (Main.masterMode)
            {
                if (NPC.localAI[0] == 1050 || NPC.localAI[0] == 1120)
                {
                    NPC.friendly = false;
                    NPC.dontTakeDamage = false;
                    var entitySource = NPC.GetSource_FromAI();
        ...
        public void DoSecondPhase(Player player)
        {
            ...
            // APPEAR AT PLAYER POS AGAIN
            if (NPC.localAI[2] >= 840 && NPC.localAI[2] <= 1250)
            {
            ...
            if (Main.masterMode)
            {
                if (NPC.localAI[2] == 850 || NPC.localAI[2] == 910 || NPC.localAI[2] == 980)
                {
                    NPC.friendly = false;
                    NPC.dontTakeDamage = false;
                    var entitySource = NPC.GetSource_FromAI();
            ...
            if (Main.masterMode)
            {
                if (NPC.localAI[2] == 1072 || NPC.localAI[2] == 1132 || NPC.localAI[2] == 1201)
                {
                    NPC.friendly = false;
                    NPC.dontTakeDamage = false;
                    var entitySource = NPC.GetSource_FromAI();
